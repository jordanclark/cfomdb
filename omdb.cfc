component {
	...
	,	boolean debug= ( request.debug ?: false )
		...
		this.lastRequest = 0;
		if ( structKeyExists( server, "omdb_lastRequest" ) ) {
			this.lastRequest = server.omdb_lastRequest;
		}
		this.config = {};
		return this;
	}
	...
	struct function apiRequest( required string api ) {
		...
		if ( this.throttle > 0 ) {
			lock name="omdb_limiter" type="exclusive" timeOut="#this.httpTimeOut#" throwOnTimeOut="false" {
